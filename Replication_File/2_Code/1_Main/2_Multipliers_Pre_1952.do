...
**# Multipliers plot: 
rename multlin1 multlin1_pre
rename multexp1 multexp1_pre
rename multrec1 multrec1_pre
...
keep multlin1_pre ub_linear_pre lb_linear_pre multexp1_pre ub_exp_pre lb_exp_pre multrec1_pre ub_rec_pre lb_rec_pre h 
tempfile tmp2
save `tmp2'
...
use `tmp1', clear
merge 1:1 h using `tmp2'


order multlin1_all ub_linear_all lb_linear_all multlin1_pre ub_linear_pre lb_linear_pre h

twoway ///
    (rarea ub_linear_all lb_linear_all h, color(blue%20)) ///
    (line multlin1_all h, lcolor(blue) lwidth(medium)) ///
    (line ub_linear_pre h, lcolor(red) lwidth(medthin) lpattern(dash)) ///
    (line lb_linear_pre h, lcolor(red) lwidth(medthin) lpattern(dash)) ///
    (scatter multlin1_pre h, c(l) clcolor(red) clwidth(medium) clpattern(dash) ///
        msymbol(circle) mcolor(red) msize(small)), ///
    xtitle("Horizon") ///
    ytitle("Coefficient and 95% CI") ///
    graphregion(color(white)) legend(off)
	
gr rename avg_mult_linear_1952, replace
	
graph export "$Folder/3_Output/Graphs/avg_mult_linear_1952.pdf", as(pdf)  replace	
	
twoway ///
    (rarea ub_exp_all lb_exp_all h, color(blue%20)) ///
    (line multexp1_all h, lcolor(blue) lwidth(medium)) ///
    (line ub_exp_pre h, lcolor(red) lwidth(medthin) lpattern(dash)) ///
    (line lb_exp_pre h, lcolor(red) lwidth(medthin) lpattern(dash)) ///
    (scatter multexp1_pre h, c(l) clcolor(red) clwidth(medium) clpattern(dash) ///
        msymbol(circle) mcolor(red) msize(small)), ///
    xtitle("Horizon") ///
    ytitle("Coefficient and 95% CI") ///
    graphregion(color(white)) legend(off)
	
gr rename avg_mult_exp_1952, replace
	
graph export "$Folder/3_Output/Graphs/avg_mult_exp_1952.pdf", as(pdf)  replace	
	
twoway  ///
    (rarea ub_rec_all lb_rec_all h, color(blue%20)) ///
    (line multrec1_all h, lcolor(blue) lwidth(medium)) ///
    (line ub_rec_pre h, lcolor(red) lwidth(medthin) lpattern(dash)) ///
    (line lb_rec_pre h, lcolor(red) lwidth(medthin) lpattern(dash)) ///
    (scatter multrec1_pre h, c(l) clcolor(red) clwidth(medium) clpattern(dash) ///
        msymbol(circle) mcolor(red) msize(small)), ///
    xtitle("Horizon") ///
    ytitle("Coefficient and 95% CI") ///
    graphregion(color(white)) legend(off)
	
gr rename avg_mult_rec_1952, replace

graph export "$Folder/3_Output/Graphs/avg_mult_rec_1952.pdf", as(pdf)  replace	
